    ...
    private void setSecurityContext() {
        if (this.process.can()) {
            ...
            FileManager.executors.execute(() -> {
                try {
                    UserSession session = this.session;
                    String[] params = new String[]{this._Role, this._Organization, this._Collabspace};
                    session.setSecurityContext("ctx::" + String.join(".", params));
